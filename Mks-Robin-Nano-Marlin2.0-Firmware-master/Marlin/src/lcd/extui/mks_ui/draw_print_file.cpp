...
// fix wang
uint16_t lv_longFilename[LONG_FILENAME_LENGTH * MAX_VFAT_ENTRIES + 1]; 
/*
Unicode      		|        UTF-8
Hexadecimal      	|        Binary
--------------------------+---------------------------------------------
      ...
      if (list_file.Sd_file_cnt == list_file.Sd_file_offset) {
        ...
        if (lv_longFilename[0] == 0)
				  strncpy(list_file.long_name[valid_name_cnt], card.filename, strlen(card.filename));
					...
					unicode_2_utf8(list_file.long_name[valid_name_cnt], lv_longFilename, LONG_FILENAME_LENGTH * MAX_VFAT_ENTRIES);
